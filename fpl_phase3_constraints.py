...
def _compute_transfer_price_ev(p_in: pd.Series,
    ...
    NaN-safe: predicted_price_change is NaN when the price model is
    unavailable or the player has a blank GW. 'float(x) or 0' does NOT
    protect against NaN because NaN is truthy in Python — it must be
    caught explicitly. NaN in total_ev propagates into the ILP objective
    coefficient and raises pulp.constants.PulpError.
    """
    def _safe_pchg(val) -> float:
        """Return 0.0 for None, NaN, or non-numeric; else the float value."""
        if val is None:
            return 0.0
        try:
            f = float(val)
    ...
    in_pchg  = _safe_pchg(p_in.get("predicted_price_change"))
    out_pchg = _safe_pchg(p_out.get("predicted_price_change"))
    price_ev = FPL_PROFIT_FRACTION * (in_pchg - out_pchg)
    return round(float(xpts_gain) + price_ev, 2)
